...
class CogVideoXDDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        timestep: int, #No longer an int. Now a list
        sample: torch.Tensor,
        eta: float = 0.0,
        ...
        # print("**********")
        # print(model_output.shape, timestep.shape, sample.shape, eta, type(variance_noise) )
        self.alphas_cumprod = self.alphas_cumprod.to('cuda')
        # print("devices: alphas", self.alphas_cumprod.device)
        # print("devices: t", timestep.device)
        ...
        valid_timesteps = torch.max(prev_timestep, torch.zeros_like(prev_timestep))
        alpha_prod_t_prev = torch.index_select(self.alphas_cumprod, 0, valid_timesteps)
        alpha_prod_t_prev = torch.repeat_interleave(alpha_prod_t_prev, temporal_scale_factor).view(broadcast_shape)
        negative_mask = torch.repeat_interleave(prev_timestep < 0, temporal_scale_factor).view(broadcast_shape)
        ##Test it with fifo loop.
        alpha_prod_t_prev = torch.where(negative_mask, self.final_alpha_cumprod, alpha_prod_t_prev)
        beta_prod_t = 1 - alpha_prod_t
        ...
        # print("beta_prod_t", beta_prod_t)
        print("sample", sample.shape, 'a_t', alpha_prod_t.shape, 'b_t', beta_prod_t.shape)
